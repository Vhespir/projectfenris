...
const LiveDot = () => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
    ...
    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', letterSpacing: '0.1em', color: 'var(--green)', textTransform: 'uppercase' }}>LIVE</span>
  </span>
)
...
const pillars = [
  ...
  },
  {
    ...
    body: 'Location-tagged field reports from members on the ground. Road conditions, power outages, local hazards. Reputation-scored and time-stamped.',
  },
  {
...
const MapPreview = () => (
  <div style={{
    ...
    overflow: 'hidden', aspectRatio: '16/9', maxHeight: '420px',
  }}>
    <svg width="100%" height="100%" viewBox="0 0 800 450" preserveAspectRatio="xMidYMid slice" style={{ display: 'block' }}>
      ...
      <path d="M120,120 L680,120 L720,200 L680,340 L500,360 L380,380 L200,360 L120,280 Z" fill="none" stroke="#1e2d1e" strokeWidth={1} />
    ...
    <div style={{ position: 'absolute', bottom: '16px', right: '16px', display: 'flex', flexDirection: 'column', gap: '4px', alignItems: 'flex-end' }}>
      {[{ shape: '●', color: '#EF4444', label: 'Severe' }, { shape: '▲', color: '#F59E0B', label: 'Moderate' }, { shape: '■', color: '#22C55E', label: 'Advisory' }].map(item => (
    ...
    <div style={{ position: 'absolute', top: '16px', right: '16px', fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--text-subtle)', letterSpacing: '0.05em' }}>
...
const TYPE_COLOR: Record<string, string> = {
  community: 'var(--info)',
  field_report: 'var(--warning)',
  self_reported_news: 'var(--green)',
}
...
function LiveFeedPreview() {
  ...
  const mockItems = [
    { category: 'FIELD REPORT', title: 'I-40 westbound closed near Flagstaff, winter storm', time: '14 min ago', color: 'var(--warning)' },
    { category: 'VERIFIED', title: 'NOAA: Winter Storm Warning issued for Coconino County', time: '32 min ago', color: 'var(--danger)' },
    { category: 'COMMUNITY', title: 'Shelter-in-place lifted for downtown Phoenix district', time: '1h ago', color: 'var(--info)' },
    { category: 'GUIDE', title: 'Vehicle winter kit: what to actually carry', time: '2h ago', color: 'var(--text-subtle)' },
  ]

  const items = posts.length >= 2
    ? posts.map(p => ({
        category: TYPE_LABEL[p.post_type] ?? p.post_type,
        title: p.title,
        time: timeAgo(p.created_at),
        color: TYPE_COLOR[p.post_type] ?? 'var(--text-subtle)',
        id: p.id,
      }))
    : mockItems
      ...
      {items.map((item, i) => (
        <div key={i} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.08em', color: item.color, textTransform: 'uppercase' }}>
              {item.category}
            </span>
          ...
          <p style={{ fontSize: '14px', color: 'var(--text)', lineHeight: 1.4 }}>{item.title}</p>
        </div>
      ))}
    </div>
  )
...
function LiveStats() {
  ...
  useEffect(() => {
    fetch('/api/events?limit=1000')
      .then(r => r.json())
      .then(data => Array.isArray(data) && setEventCount(data.length))
      .catch(() => {})
  }, [])

  const stats = [
    { value: eventCount !== null ? eventCount.toLocaleString() : '...', label: 'Active Events', mono: false },
  ...
  return (
    <div style={{ borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px', display: 'flex', gap: '48px', flexWrap: 'wrap' }}>
        {stats.map(stat => (
          <div key={stat.label}>
            <div style={{
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '12px', color: 'var(--text-subtle)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              {stat.label}
            </div>
          </div>
        ))}
  ...
  return (
    <div>
      ...
      <section style={{
        ...
        <div style={{ marginBottom: '24px' }}><LiveDot /></div>
        <h1 style={{ fontSize: 'clamp(52px, 8vw, 96px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1.0, marginBottom: '8px', color: 'var(--text)' }}>
          STAY INFORMED.
        </h1>
        <h1 style={{ fontSize: 'clamp(52px, 8vw, 96px)', fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1.0, marginBottom: '40px', color: 'var(--green)' }}>
          STAY READY.
        </h1>
        <p style={{ fontSize: 'clamp(16px, 2vw, 20px)', color: 'var(--text-muted)', maxWidth: '560px', lineHeight: 1.6, marginBottom: '48px' }}>
          The world's prepper hub. Live disaster data, crowdsourced ground truth, and practical tools.
          All in one place, owned by the community.
        </p>
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <Link to="/map" style={{
            fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '14px',
            letterSpacing: '0.02em', padding: '14px 28px', borderRadius: '8px',
            background: 'var(--green)', color: '#0A0A0A',
            display: 'inline-flex', alignItems: 'center', gap: '8px',
          }}>
            View Live Map <span style={{ fontSize: '16px' }}>→</span>
          </Link>
      ...
      <section style={{ padding: '96px 24px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ marginBottom: '48px' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--green)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '16px' }}>
            Live Data Layer
          </div>
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 600, letterSpacing: '-0.02em', marginBottom: '16px' }}>
            What's happening, right now.
          </h2>
          <p style={{ fontSize: '16px', color: 'var(--text-muted)', maxWidth: '480px', lineHeight: 1.6 }}>
            Active alerts, seismic events, federal declarations, and air quality data,
            plotted on a single map, updated every ten minutes.
          </p>
        </div>
        <MapPreview />
        <div style={{ marginTop: '16px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          {[
            ...
            <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-subtle)', letterSpacing: '0.04em' }}>
                {item.label}
              </span>
            </div>
          ))}
      ...
      {/* Platform pillars */}
      <section style={{ padding: '96px 24px', background: 'var(--surface)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1px', background: 'var(--border)' }}>
            {pillars.map((pillar, i) => (
              <div key={pillar.label} style={{ background: 'var(--surface)', padding: '40px 32px', position: 'relative' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-subtle)', letterSpacing: '0.1em', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ color: 'var(--green)', fontSize: '16px' }}>{pillar.icon}</span>
                  {pillar.label}
                </div>
                <h3 style={{ fontSize: '20px', fontWeight: 600, letterSpacing: '-0.01em', marginBottom: '12px', color: 'var(--text)' }}>
                  {pillar.title}
                </h3>
                <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: 1.7 }}>
                  {pillar.body}
                </p>
                <div style={{ position: 'absolute', top: '40px', right: '32px', fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-subtle)' }}>
                  0{i + 1}
                </div>
              </div>
            ))}
      ...
      <section style={{ padding: '96px 24px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '64px', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 600, letterSpacing: '-0.02em', marginBottom: '20px' }}>
            ...
            <Link to="/register" style={{
              ...
              background: 'var(--green)', color: '#0A0A0A', display: 'inline-block',
            }}>
              Join the Community
            </Link>
          </div>
      ...
      {/* Final CTA */}
      <section style={{ padding: '96px 24px', borderTop: '1px solid var(--border)', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '16px', color: 'var(--text-muted)', lineHeight: 1.6, marginBottom: '40px' }}>
